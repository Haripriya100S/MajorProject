...
from keras.layers import Input, Embedding, LSTM, Dense, Attention, TimeDistributed
...
questions = ["What are the departments and professional student chapters in vnrvjiet?"]
answers = ["ai ds cse ds cys automobile engineering civil engineering computer science and engineering csbs cse ai ml iot electrical and electronics engineering electronics and communication engineering electronics and instrumentation engineering humanities and sciences information technology mechanical engineering are departments whereas acm asme csi diurnalis gsdc ici ieee iei igbc isoi iste iete iucee sae turing hut vsi are student chapers of vnrvjiet"]
...
passage_sequences = np.expand_dims(passage_sequences, axis=0)
...
passage_sequences = pad_sequences(passage_sequences, padding='post', maxlen=MAX_SEQUENCE_LENGTH)
...
answer_sequences_onehot = np.zeros((len(answers), MAX_SEQUENCE_LENGTH, vocab_size), dtype='float32')
for i, answer_seq in enumerate(answer_sequences):
    for j, token_id in enumerate(answer_seq):
        answer_sequences_onehot[i, j, token_id] = 1.0

# Reshape the target answer sequences to match the model's output shape
target_sequences = answer_sequences_onehot.reshape((len(answers), MAX_SEQUENCE_LENGTH, vocab_size))
...
EPOCHS = 8
BATCH_SIZE = 16
model.fit([passage_sequences, question_sequences], target_sequences, epochs=EPOCHS, batch_size=BATCH_SIZE)
